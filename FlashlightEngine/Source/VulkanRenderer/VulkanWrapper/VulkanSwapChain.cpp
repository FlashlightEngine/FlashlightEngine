...
namespace VulkanWrapper {
    ...
    void VulkanSwapChain::CreateSwapChain() {
        ...
        if (vkCreateSwapchainKHR(m_VulkanBase.Device(), &createInfo, nullptr, &m_Handle) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create swap chain.");
        }

        vkGetSwapchainImagesKHR(m_VulkanBase.Device(), m_Handle, &imageCount, nullptr);
        m_SwapChainData.SwapChainImages.resize(imageCount);
        vkGetSwapchainImagesKHR(m_VulkanBase.Device(), m_Handle, &imageCount, m_SwapChainData.SwapChainImages.data());
    ...
    void VulkanSwapChain::DestroySwapChain() const {
        vkDestroySwapchainKHR(m_VulkanBase.Device(), m_Handle, nullptr);    
    }
    ...
    void VulkanSwapChain::CreateImageViews() {
        ...
        for (size_t i = 0; i < m_SwapChainData.SwapChainImages.size(); i++) {
            VkImageViewCreateInfo createInfo{};
            createInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
            createInfo.image = m_SwapChainData.SwapChainImages[i];
            createInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
            createInfo.format = m_SwapChainData.SwapChainImageFormat;
            ...
            if (vkCreateImageView(m_VulkanBase.Device(), &createInfo, nullptr, &m_SwapChainData.SwapChainImageViews[i]) != VK_SUCCESS) {
                throw std::runtime_error("Failed to create image views.");
            }
        }
    }

    /// @brief Destroy every image view.
    void VulkanSwapChain::DestroyImageViews() const {
        for (const auto imageView : m_SwapChainData.SwapChainImageViews) {
            vkDestroyImageView(m_VulkanBase.Device(), imageView, nullptr);
        }
    }
