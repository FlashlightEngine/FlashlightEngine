task("generate-globalheaders")

set_menu({
    usage = "xmake generate-globalheaders",
...
on_run(function ()
    ...
    local excludedFiles = {
        ["Components.hpp"] = { Define = "FL_ENTT" },
        ["ConfigCheck.hpp"] = true,
        ["Systems.hpp"] = { Define = "FL_ENTT" },
	...
	for _, modulePath in pairs(modules) do
		...
		paths[moduleName] = {
			...
			Header = head,
			HeaderGuard = "FL_GLOBAL_" .. moduleName:upper() .. "_HPP",
			Name = "FL" .. moduleName,
		...
		if (os.isdir(modulePath .. "/Components")) then
			paths[moduleName .. "_Components"] = {
				Excludes = table.copy(excludedFiles),
				Header = head,
				HeaderGuard = "FL_" .. moduleName:upper() .. "_COMPONENTS_HPP",
				Name = "FL" .. moduleName .. " components",
		...
		if (os.isdir(modulePath .. "/Systems")) then
			paths[moduleName .. "_Systems"] = {
				Excludes = table.copy(excludedFiles),
				Header = head,
				HeaderGuard = "FL_" .. moduleName:upper() .. "_SYSTEMS_HPP",
				Name = "FL" .. moduleName .. " systems",
	...
	paths["Core"].Excludes["AppEntitySystemComponent.hpp"] = { Define = "FL_ENTT" }
	paths["Core"].Excludes["EnttSystemGraph.hpp"] = { Define = "FL_ENTT" }
	paths["Core"].Excludes["EnttWorld.hpp"] = { Define = "FL_ENTT" }
	...
	paths["VulkanWrapper"] = {
		...
		HeaderGuard = "FL_VULKANRENDERER_WRAPPER_HPP",
		Name = "Vulkan wrapper",
		SearchDir = "Include/Flashlight/VulkanRenderer/Wrapper",
		Target = "Include/Flashlight/VulkanRenderer/Wrapper.hpp"
	}
